...
public class PuzzleController : MonoBehaviour
{
    ...
    public PuzzleState ProcessPuzzle (Vector3 _pointerPosition, bool _dragInput, float _rotationDirection)
    {
        ...
        if (_dragInput  &&  currentPiece < 0) 
        {
            ...
            if (currentPiece >= 0) 
            {
                ...
                // Don't apply offset for groups
                 pieceCenterOffset = (!currentGroup) ? pieces [currentPiece].GetPieceCenterOffset() :  Vector2.zero;
            ...
            if (decomposeSides.Count > 0)
                for (int i = 0; i < pieces.Length; i++)
                    ...
                    // Setup decompose area size
                        ...
                        // Set random Z-rotation (if needed)
                        if (randomizeRotation)
                            pieces[i].transform.RotateAround(pieces[i].renderer.bounds.center, Vector3.forward, gradualRotation ? rotationSpeed * Random.Range(0, 360/(int)rotationSpeed) : rotationSpeed * Random.rotation.z);
	...
	public bool IsAssembled () 
	{
        ...
        sucessPiece = 16 - movedPieces.Count;
    ...
    public SpriteRenderer GenerateBackground(Texture2D _image, bool _adjustToPuzzle = true, bool _centerAnchoring = true)
    {
        if (_image == null)
            {
    ...
    public PuzzleAnchor SetAnchor(PuzzleAnchor _anchor)
    {
        ...
        {
            Debug.Log("<b>PuzzleController: </b> <i> Can't set puzzle anchor</i>. Please ensure that you've called <i>Prepare</i> function heretofore.</br> Or just press 'RECALCULATE'", gameObject);
            return anchoring;
        }
        ...
        Transform background = thisTransform.GetChild(0);
        if (background.tag == "Puzzle_Background")
            background.position = new Vector3(background.position.x - anchorOffset.x, anchorOffset.y + background.position.y, background.position.z);
    ...
    public void AlignWithCameraCenter(Camera _camera, bool _centerAnchoring, bool changeCameraSize = true,  float _cameraSizeTuner = 0)
    {
        ...
        if (_camera)
        {
            Vector3 screenviewCenter = _camera.ViewportToWorldPoint(new Vector3(0.5f, 0.5f, _camera.nearClipPlane));
            screenviewCenter.z = 0;
            transform.position = _centerAnchoring ? screenviewCenter : new Vector3(screenviewCenter.x - puzzleBounds.center.x, screenviewCenter.y - puzzleBounds.center.y, transform.position.z);
            // CalculateBounds();
    ...
    public void FitToScreen(Camera _camera, ScreenAnchor _anchor)
    {
        if (_anchor == ScreenAnchor.None) return;

        // Adjust size
        _camera.transform.position = new Vector3(puzzleBounds.center.x, puzzleBounds.center.y, _camera.transform.position.z);
        Vector3 boundsInView = _camera.WorldToViewportPoint(puzzleBounds.max);
        ...
        float diffInView = (0.5f - boundsInView.y);
        float diffInScene = (puzzleBounds.center.y - puzzleBounds.min.y * positionModifier);

        _camera.transform.position = new Vector3(_camera.transform.position.x, _camera.transform.position.y - boundsInView.y * diffInScene / diffInView, _camera.transform.position.z);
    }
